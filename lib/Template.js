const { parse$requests, composeImports, mapDependenciesToVariables } = require("./utils")
    ...
    async _getOutput() {
        ...
        Object.values(this._placeholders).forEach(placeholder => {
            placeholder.parts.forEach(part => {
                const { body, dependencies } = parse$requests(part.str)
                if (dependencies.length) {
                    const mappedDependencies = mapDependenciesToVariables(this._options.imports, dependencies)
                    const { imports, declarations } = composeImports(mappedDependencies)
