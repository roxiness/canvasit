...
const { Placeholder, Template, PlaceholderFragment } = require('./fileMerger')
...
test('Template derives template from template string', t => {
    const template = new Template(templateStr, { ext: 'html' })
...
test('template class can create an output from its template', t => {
    const template = new Template(templateStr, { ext: 'html' })
    const output = template._getOutput()
    t.regex(output, /<script>/s,)
})

test('a placeholder can push and unshift strings', t => {
    const template = new Template(templateStr, { ext: 'html' })
