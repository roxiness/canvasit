 ...
 */
function $request(...importPath) {
  return _eval => `__$REQUEST${JSON.stringify({ importPath, _eval })}$REQUEST__`
}

/**
 * replaces $request placeholders with actual calls and
 * returns the body and a list of aliased dependencies found
 * @param {string} body 
 * @returns {{body: string, dependencies: string[]}}
 */
function parse$requests(body) {
  const dependencies = []
  body = body.replace(/__\$REQUEST(.+)\$REQUEST__/g, (_, req) => {
    const { importPath, _eval } = JSON.parse(req)
